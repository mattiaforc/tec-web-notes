...
public class FeedDb implements Serializable {
	...
	public List<Feed> find(String category){
		...
		for ( Feed feed : feeds ) {
			if(feed.getCategory().equalsIgnoreCase(category)) {
				res.add(feed);
			}
		}
		return res;
	...
	public List<String> getCategories(String categoryStartingWith) {
		...
		.filter(t -> t
				.getCategory()
				.substring(0, Math.min(t.getCategory().length(), categoryStartingWith.length()))
				.equalsIgnoreCase(categoryStartingWith))
		.map(t -> t.getCategory())
